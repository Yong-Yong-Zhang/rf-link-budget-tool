...
<head>
    ...
    <title>RF Link Budget Calculator (v2.0.0-rc)</title>
    <style>
        /* RF Link Budget Calculator v2.0.0-rc CSS */

        /* --- 1. Global Reset & Dark Theme --- */
        body { margin: 0; padding: 0; font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; background-color: #2B2B2B; color: #E0E0E0; height: 100vh; overflow: hidden; user-select: none; }
        * { box-sizing: border-box; }

        /* --- 2. Main Layout --- */
        .main-container { display: flex; height: 100vh; width: 100vw; }
        .toolbox-frame { width: 240px; background-color: #1E1E1E; border-right: 1px solid #444; display: flex; flex-direction: column; padding: 10px; overflow-y: auto; flex-shrink: 0; }
        .right-frame { flex-grow: 1; display: flex; flex-direction: column; padding: 10px; background-color: #252526; overflow: hidden; }

        /* --- 3. Toolbox Styling --- */
        .toolbox-section { margin-bottom: 15px; }
        fieldset { border: 1px solid #555; border-radius: 5px; padding: 8px; margin: 0; }
        legend { color: #87CEFA; font-size: 0.9em; font-weight: bold; padding: 0 5px; }
        button { background-color: #3C3C3C; color: #E0E0E0; border: 1px solid #555; border-radius: 3px; padding: 6px 10px; margin: 3px 0; cursor: pointer; font-size: 13px; transition: background-color 0.2s, border-color 0.2s; width: 100%; text-align: left; }
        button:hover { background-color: #505050; border-color: #777; }
        button:active { background-color: #2D2D30; }
        .primary-btn { background-color: #007ACC !important; border-color: #007ACC !important; font-weight: bold; }
        .primary-btn:hover { background-color: #0098FF !important; }
        .toolbox-hint { margin-top: auto; font-size: 0.8em; color: #888; background: #252526; padding: 8px; border-radius: 4px; border: 1px solid #333; }
        .toolbox-hint ul { padding-left: 18px; margin: 5px 0 0 0; }

        /* --- 4. Top Bar --- */
        .top-bar { display: flex; gap: 10px; margin-bottom: 10px; flex-wrap: wrap; align-items: flex-start; }
        .system-settings-frame { background: #333; padding: 8px; border-radius: 4px; display: flex; align-items: center; border: 1px solid #444; }
        .system-settings-frame input { width: 60px; background: #222; border: 1px solid #555; color: #FFF; padding: 4px; margin-left: 5px; text-align: center; }
        .tab-control { flex-grow: 1; background: #333; border-radius: 4px; border: 1px solid #444; overflow: hidden; display: flex; flex-direction: column; }
        .tab-buttons { display: flex; background: #252526; border-bottom: 1px solid #444; }
        .tab-button { background: #2D2D30; border: none; border-right: 1px solid #444; padding: 8px 15px; width: auto; text-align: center; border-radius: 0; margin: 0; }
        .tab-button.active { background: #3E3E42; color: #FFF; font-weight: bold; border-bottom: 2px solid #007ACC; }
        .tab-content-area { padding: 8px; }
        .tab-content { display: none; align-items: center; font-size: 0.9em; }
        .tab-content.active { display: flex; }
        .tab-content input { background: #222; border: 1px solid #555; color: #FFF; padding: 4px; margin-left: 5px; width: 70px; }
        .control-frame { display: flex; gap: 5px; align-items: flex-start; }
        .control-frame button { width: auto; padding: 8px 12px; }
        .tool-button { width: auto; padding: 8px 12px; }

        /* --- 5. Canvas --- */
        .canvas-frame { flex-grow: 1; display: flex; flex-direction: column; padding: 0; margin-bottom: 10px; overflow: hidden; background: #1E1E1E; }
        .canvas-wrapper { flex-grow: 1; position: relative; overflow: hidden; width: 100%; height: 100%; }
        canvas { display: block; background-image: radial-gradient(#333 1px, transparent 1px); background-size: 20px 20px; background-color: #1E1E1E; width: 100%; height: 100%; }
        canvas.panning { cursor: grab; }

        /* --- 6. Results & Log --- */
        .bottom-panel-container { height: 200px; display: flex; gap: 10px; flex-shrink: 0; }
        .result-frame, .calc-log-frame { flex: 1; display: flex; flex-direction: column; overflow: hidden; background: #1E1E1E; }
        pre { background: #222; color: #DDD; padding: 10px; margin: 0; overflow: auto; flex-grow: 1; font-family: 'Consolas', 'Monaco', monospace; font-size: 12px; border-top: 1px solid #444; }
        ...
        .modal-content { background: #2D2D30; width: 750px; max-height: 85vh; border: 1px solid #555; border-radius: 5px; display: flex; flex-direction: column; box-shadow: 0 4px 15px rgba(0,0,0,0.5); }
        .modal-header { padding: 10px 15px; background: #3E3E42; border-bottom: 1px solid #555; display: flex; justify-content: space-between; align-items: center; }
        .modal-header h3 { margin: 0; }
        .modal-header button { width: auto; background: none; border: none; font-size: 1.5em; padding: 0; }
        .modal-body { padding: 15px; overflow-y: auto; flex-grow: 1; }
        .modal-specs-container { display: flex; gap: 15px; height: 450px; }
        .freq-list-section { width: 150px; display: flex; flex-direction: column; }
        .freq-list-section select { flex-grow: 1; background: #222; color: #FFF; border: 1px solid #555; margin-bottom: 5px; }
        .freq-btn-frame { display: flex; gap: 5px; }
        .spec-editor-section { flex-grow: 1; border: 1px solid #444; padding: 10px; background: #252526; overflow-y: auto; }
        
        .spec-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 10px; align-items: center; }
        .spec-grid label { text-align: right; font-size: 0.9em; color: #AAA; }
        .spec-grid input { background: #333; border: 1px solid #555; color: #FFF; padding: 4px; width: 100%; }
        .array-calc-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 10px; margin-bottom: 10px; }
        .array-calc-grid label { display: block; font-size: 0.85em; color: #AAA; margin-bottom: 2px; }
        .array-calc-grid input { width: 100%; background: #333; border: 1px solid #555; color: #FFF; padding: 4px; }

        .split-specs-container { display: flex; gap: 15px; height: 100%; }
        .split-specs-col { flex: 1; border: 1px solid #444; padding: 10px; border-radius: 4px; background: #2D2D30; display: flex; flex-direction: column; overflow-y: auto; }
        .split-specs-col h4 { margin: 0 0 10px 0; color: #87CEFA; border-bottom: 1px solid #555; padding-bottom: 5px; text-align: center; font-size: 1.1em; position: sticky; top: 0; background: #2D2D30; z-index: 1;}
        
        .merged-list-item { font-size: 0.85em; padding: 4px 0; border-bottom: 1px solid #444; color: #CCC; }
        .merged-list-item:last-child { border-bottom: none; }
        .merged-list-item b { color: #FFF; }
        .merged-list-item span { color: #AAA; font-size: 0.9em; margin-left: 5px; }

        .modal-footer { padding: 10px 15px; background: #3E3E42; border-top: 1px solid #555; display: flex; justify-content: flex-end; gap: 10px; }
        .modal-footer button { width: auto; min-width: 80px; }

        /* --- 8. Context Menu --- */
        .context-menu { position: fixed; background: #2D2D30; border: 1px solid #555; box-shadow: 2px 2px 10px rgba(0,0,0,0.5); z-index: 2000; min-width: 150px; }
        .context-menu ul { list-style: none; margin: 0; padding: 5px 0; }
        .context-menu li { padding: 5px 15px; cursor: pointer; font-size: 0.9em; }
        .context-menu li:hover { background: #3E3E42; }
        .context-menu li.danger:hover { background: #800; color: white; }
        .context-menu .separator { height: 1px; background: #444; margin: 3px 0; padding: 0; }

        /* Icons */
        .toolbox-section > fieldset > button { background-repeat: no-repeat; background-position: 10px center; background-size: 20px 20px; padding-left: 38px; position: relative; }
        #add-lna { background-image: url("data:image/svg+xml,%3Csvg width='24' height='24' viewBox='0 0 24 24' xmlns='http://www.w3.org/2000/svg'%3E%3Cpath d='M4 4 L20 12 L4 20 Z' fill='none' stroke='%23E0E0E0' stroke-width='2' stroke-linejoin='round'/%3E%3Cpath d='M8 9 L8 15' stroke='%23E0E0E0' stroke-width='2'/%3E%3C/svg%3E"); }
        #add-pa { background-image: url("data:image/svg+xml,%3Csvg width='24' height='24' viewBox='0 0 24 24' xmlns='http://www.w3.org/2000/svg'%3E%3Cpath d='M4 4 L20 12 L4 20 Z' fill='none' stroke='%23E0E0E0' stroke-width='2' stroke-linejoin='round'/%3E%3C/svg%3E"); }
        #add-mixer { background-image: url("data:image/svg+xml,%3Csvg width='24' height='24' viewBox='0 0 24 24' xmlns='http://www.w3.org/2000/svg'%3E%3Ccircle cx='12' cy='12' r='9' fill='none' stroke='%23E0E0E0' stroke-width='2'/%3E%3Cpath d='M7 7 L17 17 M7 17 L17 7' stroke='%23E0E0E0' stroke-width='2'/%3E%3C/svg%3E"); }
        #add-ic { background-image: url("data:image/svg+xml,%3Csvg width='24' height='24' viewBox='0 0 24 24' xmlns='http://www.w3.org/2000/svg'%3E%3Crect x='5' y='5' width='14' height='14' fill='none' stroke='%23E0E0E0' stroke-width='2'/%3E%3Cpath d='M5 8H2 M5 12H2 M5 16H2 M19 8H22 M19 12H22 M19 16H22 M8 5V2 M12 5V2 M16 5V2 M8 19V22 M12 19V22 M16 19V22' stroke='%23E0E0E0' stroke-width='2'/%3E%3C/svg%3E"); }
        #add-filter { background-image: url("data:image/svg+xml,%3Csvg width='24' height='24' viewBox='0 0 24 24' xmlns='http://www.w3.org/2000/svg'%3E%3Crect x='2' y='6' width='20' height='12' rx='2' fill='none' stroke='%23E0E0E0' stroke-width='2'/%3E%3Cpath d='M6 12 Q9 9 12 12 T18 12' fill='none' stroke='%23E0E0E0' stroke-width='2'/%3E%3C/svg%3E"); }
        #add-atten { background-image: url("data:image/svg+xml,%3Csvg width='24' height='24' viewBox='0 0 24 24' xmlns='http://www.w3.org/2000/svg'%3E%3Cpath d='M2 12 L5 6 L10 18 L15 6 L20 18 L22 12' fill='none' stroke='%23E0E0E0' stroke-width='2' stroke-linejoin='round'/%3E%3C/svg%3E"); }
        #add-div2 { background-image: url("data:image/svg+xml,%3Csvg width='24' height='24' viewBox='0 0 24 24' xmlns='http://www.w3.org/2000/svg'%3E%3Crect x='2' y='4' width='10' height='16' fill='none' stroke='%23E0E0E0' stroke-width='2'/%3E%3Cpath d='M12 12 L18 8 M12 12 L18 16' stroke='%23E0E0E0' stroke-width='2'/%3E%3C/svg%3E"); }
        #add-div4 { background-image: url("data:image/svg+xml,%3Csvg width='24' height='24' viewBox='0 0 24 24' xmlns='http://www.w3.org/2000/svg'%3E%3Crect x='2' y='4' width='8' height='16' fill='none' stroke='%23E0E0E0' stroke-width='2'/%3E%3Cpath d='M10 12 L16 6 M10 12 L16 10 M10 12 L16 14 M10 12 L16 18' stroke='%23E0E0E0' stroke-width='1.5'/%3E%3C/svg%3E"); }
        #add-trace { background-image: url("data:image/svg+xml,%3Csvg width='24' height='24' viewBox='0 0 24 24' xmlns='http://www.w3.org/2000/svg'%3E%3Cpath d='M2 14 H22' stroke='%23E0E0E0' stroke-width='2'/%3E%3Cpath d='M2 10 H22' stroke='%23E0E0E0' stroke-width='2' stroke-dasharray='4 2'/%3E%3C/svg%3E"); }
        #add-airloss { background-image: url("data:image/svg+xml,%3Csvg width='24' height='24' viewBox='0 0 24 24' xmlns='http://www.w3.org/2000/svg'%3E%3Cpath d='M2 12 Q6 4 10 12 T18 12' fill='none' stroke='%23E0E0E0' stroke-width='2' stroke-dasharray='2 2'/%3E%3C/svg%3E"); }
        #add-antenna { background-image: url("data:image/svg+xml,%3Csvg width='24' height='24' viewBox='0 0 24 24' xmlns='http://www.w3.org/2000/svg'%3E%3Cpath d='M12 22 L12 10 M12 10 L4 2 M12 10 L20 2' stroke='%23E0E0E0' stroke-width='2'/%3E%3C/svg%3E"); }
        #add-array { background-image: url("data:image/svg+xml,%3Csvg width='24' height='24' viewBox='0 0 24 24' xmlns='http://www.w3.org/2000/svg'%3E%3Crect x='3' y='3' width='6' height='6' fill='%23666'/%3E%3Crect x='15' y='3' width='6' height='6' fill='%23666'/%3E%3Crect x='3' y='15' width='6' height='6' fill='%23666'/%3E%3Crect x='15' y='15' width='6' height='6' fill='%23666'/%3E%3C/svg%3E"); }
        #load-component { background-image: url("data:image/svg+xml,%3Csvg width='24' height='24' viewBox='0 0 24 24' xmlns='http://www.w3.org/2000/svg'%3E%3Cpath d='M4 18v3h16v-3' fill='none' stroke='%23E0E0E0' stroke-width='2' stroke-linecap='round'/%3E%3Cpath d='M12 14V3m0 0l-4 4m4-4l4 4' fill='none' stroke='%23E0E0E0' stroke-width='2' stroke-linecap='round' stroke-linejoin='round'/%3E%3C/svg%3E"); }
        #merge-components { background-image: url("data:image/svg+xml,%3Csvg width='24' height='24' viewBox='0 0 24 24' xmlns='http://www.w3.org/2000/svg'%3E%3Crect x='2' y='2' width='8' height='8' stroke='%23E0E0E0' fill='none'/%3E%3Crect x='14' y='14' width='8' height='8' stroke='%23E0E0E0' fill='none'/%3E%3Cpath d='M10 10 L14 14' stroke='%23E0E0E0' stroke-width='2'/%3E%3C/svg%3E"); }
    </style>
</head>
<body>
    ...
    <div class="main-container">
        <div class="toolbox-frame">
            <div style="text-align: center; margin: 5px 0 10px 0; padding-bottom: 10px; border-bottom: 2px solid #444;">
                <h3 style="margin: 0; color: #87CEFA; font-size: 18px; font-weight: bold;">RF Link Budget</h3>
                <div style="font-size: 13px; color: #AAAAAA; margin-top: 3px;"> @ Yong (v2.0.0-rc)</div>
            </div>
            ...
            <div class="toolbox-section">
                <fieldset>
                    ...
                    <button id="add-ic">Add IC</button>
                </fieldset>
            </div>
            ...
            <div class="toolbox-section">
                <fieldset>
                    ...
                    <button id="add-airloss">Add Path Loss</button>
                    <hr style="border-top: 1px solid #495057; margin: 10px 5px;">
            ...
            <div class="toolbox-hint">
                <strong>Tips (v2.0.0-rc):</strong>
                <ul>
                    <li><b>Drag to Select Multiple</b></li>
                    <li><b>Del</b> Key: Delete Selected</li>
        ...
        <div class="right-frame">
            ...
            <div class="top-bar">
                ...
                <div class="control-frame">
                    <button id="clear-lines-button">Clear Lines</button>
                    <button id="clear-button">Clear All</button>
                    <button id="calc-button" class="primary-btn">Calculate</button>
                </div>
            </div>

            <fieldset class="canvas-frame">
                <legend>Block Diagram (Scroll to Zoom / Middle Click to Pan)</legend>
    ...
    <div id="block-context-menu" class="context-menu" style="display: none;">
        <ul>
            ...
            <li id="menu-unmerge-comp" style="display:none;">Unmerge Component</li> 
            <li id="menu-delete-comp" class="danger">Delete Component</li>
    ...
    <div id="line-context-menu" class="context-menu" style="display: none;">
        <ul>
            <li id="menu-delete-line" class="danger">Delete Connection</li>
            <li class="separator"></li>
    ...
    <script>
        /*
         * RF Link Budget Calculator (Web App v2.0.0-rc)
         * - Fix: Visual glitch in component box drawing (drawRoundedRect fixed).
         * - Feature: Enhanced Saturation Warning (Red Border + "SAT" Badge + Clamped Pout).
         */
        ...
        // Selection Box Data
        let selectionData = { isSelecting: false, startX: 0, startY: 0, currentX: 0, currentY: 0 };
        ...
        class RFComponent {
            ...
            calculateSpecs(freqStr, mode, specsDict) {
                ...
                if (this.isPassive) {
                    ...
                    storage['total_ip1db_dbm'] = parseFloat(specsDict.total_ip1db_dbm || 99.0);
            ...
            getRawSpecsForFreq(freqStr, mode) {
                ...
                if (this.isPassive) return { 'loss_db': specs.loss_db || 0.0 };
                    ...
                    if(this.isMerged){
                        raw['total_ip1db_dbm'] = specs.total_ip1db_dbm || 99.0;
            ...
            getDisplaySpecsLines(freq, mode) {
                ...
                if (this.isPassive) {
                    ...
                    if (this.isMerged) {
                        ...
                        lines.push(`G: ${formatNum(specs.gain_db, 1)} dB`);
                        lines.push(`NF: ${formatNum(specs.nf_db, 1)} dB`);
                        if (mode === "TX" && !this.isArray) lines.push(`OP1dB: ${formatNum(specs.op1db_dbm, 1)} dBm`);
                    }
                }
                return lines;
            }
            ...
            toDict() {
                ...
                return {
                    ...
                    'specs_by_freq': specsToSave, 'isMerged': this.isMerged, 
            ...
            static fromDict(data) {
                ...
                if (data.displayLists) comp.displayLists = data.displayLists;
                return comp;
            }
        }
        ...
        class RFLInkBudget {
            ...
            calculate(calcFreqStr, mode = "TX") {
                ...
                this.calcLog.push(`============================`);
                this.chain.forEach(c => { c.runtimeResults = null; c.isCompressed = false; });
                ...
                for (let i = 0; i < this.chain.length; i++) {
                    ...
                    let stage_pout_dbm = cumulative_pout_dbm + stage_gain_db;
                    
                    if (mode === "TX" && !comp.isPassive && !comp.isSystem && !comp.isArray) {
                        if (stage_pout_dbm > stage_op1db_dbm) {
                            stage_pout_dbm = stage_op1db_dbm; // Clamp Output
                            comp.isCompressed = true;
                            this.calcLog.push(`  [WARNING] Saturated! Clamped to OP1dB: ${formatNum(stage_op1db_dbm, 1)} dBm`);
                        }
                    }
                    
                    const stage_pin_dbm = cumulative_pout_dbm;
                    cumulative_pout_dbm = stage_pout_dbm;
                    ...
                    this.calcLog.push(`  G_cum: ${formatNum(stage_pin_dbm, 2)} + ${formatNum(stage_gain_db, 2)} = ${formatNum(stage_pout_dbm, 2)} dBm`);
                    ...
                    if (!comp.isPassive && !comp.isSystem && !comp.isArray) { 
                        last_active_index = i; 
                        const current_total_gain_db = linear_to_db(cumulative_gain_linear);
                        const stage_referred_ip1db_dbm = stage_op1db_dbm - current_total_gain_db;
                        const stage_referred_ip1db_mw = dbm_to_mw(stage_referred_ip1db_dbm);
                        if (stage_referred_ip1db_mw < system_ip1db_mw) {
                            system_ip1db_mw = stage_referred_ip1db_mw;
                        }
                    }
                ...
                let final_op1db_dbm = 99.0;
                if (last_active_index !== -1) {
                    const lastActiveComp = this.chain[last_active_index];
                    const activeSpecs = lastActiveComp.getSpecsForFreq(calcFreqStr, mode);
                    let calc_op1db = activeSpecs['op1db_dbm'];
                    for (let j = last_active_index + 1; j < this.chain.length; j++) {
                        const subComp = this.chain[j];
                        const subSpecs = subComp.getSpecsForFreq(calcFreqStr, mode);
                        calc_op1db += subSpecs['gain_db'];
                    }
                    final_op1db_dbm = calc_op1db;
                } else {
                    final_op1db_dbm = 99.0; 
                }

                let system_ip1db_dbm = mw_to_dbm(system_ip1db_mw);
                if (system_ip1db_mw === Infinity) system_ip1db_dbm = 99.0;
                ...
                this.results['chain'] = {
                    ...
                    'total_op1db_dbm': final_op1db_dbm, 
                    'total_ip1db_dbm': system_ip1db_dbm, 
            ...
            getReport(calcFreqStr, mode = "TX") {
                ...
                if (mode === "TX") {
                    ...
                    report_str += `  System OP1dB:               ${formatNum(chain_res['total_op1db_dbm'], 2).padStart(7)} dBm\n`;
                    report_str += `  System IP1dB:               ${formatNum(chain_res['total_ip1db_dbm'], 2).padStart(7)} dBm\n`;
                    report_str += "  --------------------------------------------------\n";
        ...
        function onKeyDown(e) {
            if (e.key === "Delete") {
                ...
                if (selected.length > 0 && confirm(`Delete ${selected.length} selected components?`)) {
                    blocks = blocks.filter(b => !b.isSelected);
                    const deletedIds = new Set(selected.map(b => b.id));
                    [connections_TX, connections_RX].forEach(map => {
                        const toDelete = [];
                        for (const [from, to] of map.entries()) {
                            if (deletedIds.has(from) || deletedIds.has(to)) toDelete.push(from);
                        }
            ...
            if (e.ctrlKey && (e.key === 'd' || e.key === 'D')) {
                ...
                if (selected.length > 0) {
                    selected.forEach(original => {
                         ...
                         newComp.x = original.x + 30; 
                         newComp.y = original.y + 30;
        ...
        function executeMerge(selectedIds) {
            ...
            try { sortedTX = topologicalSortComponents(selectedComps, connections_TX); } catch (e) { console.log("TX Sort failed or empty: " + e.message); }
            try { sortedRX = topologicalSortComponents(selectedComps, connections_RX); } catch (e) { console.log("RX Sort failed or empty: " + e.message); }

            if (sortedTX.length === 0 && sortedRX.length === 0) {
                alert("Merge Error: Selected components must form a valid chain in at least one mode (TX or RX).");
                return;
            }

            const allFreqs = new Set(); 
            selectedComps.forEach(c => c.getAvailableFreqs().forEach(f => allFreqs.add(f)));
            const validFreqs = [];
            for (const freq of allFreqs) {
                let isFreqCommon = true;
                for (const comp of selectedComps) { 
                    if (!comp.getSpecsForFreq(freq, "TX") || !comp.getSpecsForFreq(freq, "RX")) { 
                        isFreqCommon = false; break; 
                    } 
                }
                if (isFreqCommon) validFreqs.push(freq);
            }
            if (validFreqs.length === 0) { alert("Merge Failed: No common frequencies found."); return; }

            const nameRef = (sortedTX.length > 0) ? sortedTX[0] : (sortedRX.length > 0 ? sortedRX[0] : selectedComps[0]);
            const newName = prompt("Enter name for merged component:", "Merged-" + nameRef.name); if (!newName) return; 

            const newSpecsByFreq = {}; 
            const tempCalculator = new RFLInkBudget();

            const calcSpecs = (chain, freq, mode) => {
                if (!chain || chain.length === 0) return { 'gain_db': 0, 'nf_db': 0, 'op1db_dbm': 99, 'total_ip1db_dbm': 99 };
                tempCalculator.setChain(chain); 
                tempCalculator.setSystemParams(-100, -100); 
                try {
                    tempCalculator.calculate(freq, mode);
                    const res = tempCalculator.results.chain;
                    return { 
                        'gain_db': res.total_gain_db, 
                        'nf_db': res.total_nf_db, 
                        'op1db_dbm': res.total_op1db_dbm,
                        'total_ip1db_dbm': res.total_ip1db_dbm,
                        'active_gain_db': res.total_active_gain_db, 
                        'passive_gain_db': res.total_passive_gain_db, 
                        'system_gain_db': res.total_system_gain_db 
                    };
                } catch(e) { return { 'gain_db': 0, 'nf_db': 0, 'op1db_dbm': 99, 'total_ip1db_dbm': 99 }; }
            };

            for (const freq of validFreqs) {
                newSpecsByFreq[freq] = {
                    "TX": calcSpecs(sortedTX, freq, "TX"),
                    "RX": calcSpecs(sortedRX, freq, "RX")
                };
            }

            let txRewire = null, rxRewire = null;
            if (sortedTX.length > 0) {
                const start = sortedTX[0]; const end = sortedTX[sortedTX.length - 1];
                let inKey = null, outKey = connections_TX.get(end.id);
                for (const [from, to] of connections_TX.entries()) if (to === start.id) inKey = from;
                txRewire = { in: inKey, out: outKey };
            }

            if (sortedRX.length > 0) {
                const start = sortedRX[0]; const end = sortedRX[sortedRX.length - 1];
                let inKey = null, outKey = connections_RX.get(end.id);
                for (const [from, to] of connections_RX.entries()) if (to === start.id) inKey = from;
                rxRewire = { in: inKey, out: outKey };
            }

            const mergedComp = new RFComponent(newName, false, false, newSpecsByFreq);
            const posRef = (currentCalcMode === "TX" && sortedTX.length > 0) ? sortedTX[0] : (sortedRX.length > 0 ? sortedRX[0] : selectedComps[0]);
            mergedComp.x = posRef.x; mergedComp.y = posRef.y; 
            mergedComp.isMerged = true;
            
            mergedComp.displayLists = { TX: sortedTX.map(c => c.toDict()), RX: sortedRX.map(c => c.toDict()) };
            mergedComp.childrenData = selectedComps.map(c => c.toDict()); 
            ...
            [connections_TX, connections_RX].forEach(map => {
                const toDelete = [];
                for (const [from, to] of map.entries()) {
                    if (selectedIdsSet.has(from) || selectedIdsSet.has(to)) toDelete.push(from);
                }
                toDelete.forEach(k => map.delete(k));
            });

            if (txRewire) {
                if (txRewire.in) connections_TX.set(txRewire.in, mergedComp.id);
                if (txRewire.out) connections_TX.set(mergedComp.id, txRewire.out);
            }
            if (rxRewire) {
                if (rxRewire.in) connections_RX.set(rxRewire.in, mergedComp.id);
                if (rxRewire.out) connections_RX.set(mergedComp.id, rxRewire.out);
            }
        ...
        // --- FIXED: Use backticks (`) instead of quotes (') for HTML strings with variables ---
        function modalToggleSpecEntries(freqSelected) {
            ...
            if (editingComp.isPassive) {
                    ...
                    splitContainer.className = 'split-specs-container';
                    
                    const txCol = document.createElement('div');
                    txCol.className = 'split-specs-col';
                    txCol.innerHTML = `<h4>TX Settings</h4>`;
                    
                    const rxCol = document.createElement('div');
                    rxCol.className = 'split-specs-col';
                    rxCol.innerHTML = `<h4>RX Settings</h4>`;
                    
                    if (editingComp.isMerged) {
                        const generateChildList = (mode) => {
                            let html = '<div style="margin-bottom:10px; font-size:0.85em;">';
                            html += `<div style="color:#AAA; margin-bottom:5px; font-weight:bold; border-bottom:1px solid #555; padding-bottom:2px;">${mode} Components:</div>`;
                            
                            const list = (editingComp.displayLists && editingComp.displayLists[mode]) ? editingComp.displayLists[mode] : editingComp.childrenData;
                            
                            if (!list || list.length === 0) {
                                html += '<div style="color:#800; padding:5px;">(No Chain Data)</div>';
                            } else {
                                list.forEach(childData => {
                                    const tempComp = RFComponent.fromDict(childData);
                                    const raw = tempComp.getRawSpecsForFreq(editingCurrentFreq, mode);
                                    let specsStr = "";
                                    if (tempComp.isPassive) {
                                        specsStr = `<span style="color:#A8E6CF">L:${formatNum(raw.loss_db||0, 1)}</span>`;
                                    } else {
                                        specsStr = `<span style="color:#BDE0FE">G:${formatNum(raw.gain_db||0, 1)}</span> <span style="color:#FFD700">NF:${formatNum(raw.nf_db||0, 1)}</span>`;
                                        if (mode === "TX") specsStr += ` <span style="color:#FFABAB">P1:${formatNum(raw.op1db_dbm||99, 1)}</span>`;
                                    }
                                    html += `<div class="merged-list-item"><b>${tempComp.name}</b> ${specsStr}</div>`;
                                });
                            }
                            html += '</div>';
                            return html;
                        ...
                        const getSummaryHTML = (raw, mode) => {
                            const p1dbVal = (mode === "RX") ? "---" : formatNum(raw.op1db_dbm || 99, 1);
                            const ip1dbVal = (mode === "RX") ? "---" : formatNum(raw.total_ip1db_dbm || 99, 1); 
                            
                            return `
                            <div style="background:#2D2D30; border:1px solid #555; border-radius:4px; padding:8px; margin-top:auto; font-size:0.9em; line-height:1.5;">
                                <div style="color:#87CEFA; border-bottom:1px solid #555; margin-bottom:5px; font-weight:bold;">Gain Breakdown (dB):</div>
                                <div style="display:flex; justify-content:space-between;"><span>Active Gain:</span> <span style="color:#FFF">${formatNum(raw.active_gain_db||0, 1)}</span></div>
                                <div style="display:flex; justify-content:space-between;"><span>Passive Gain:</span> <span style="color:#FFF">${formatNum(raw.passive_gain_db||0, 1)}</span></div>
                                <div style="display:flex; justify-content:space-between; border-top:1px dashed #444; margin-top:2px; padding-top:2px;"><span>Total Gain:</span> <span style="color:#87CEFA; font-weight:bold;">${formatNum(raw.gain_db||0, 1)}</span></div>
                                
                                <div style="color:#FFD700; border-bottom:1px solid #555; margin:10px 0 5px 0; font-weight:bold;">Linearity & Noise:</div>
                                <div style="display:flex; justify-content:space-between;"><span>Total NF (dB):</span> <span style="color:#FFF">${formatNum(raw.nf_db||0, 1)}</span></div>
                                <div style="display:flex; justify-content:space-between;"><span>Output P1dB (dBm):</span> <span style="color:#FFABAB">${p1dbVal}</span></div>
                                <div style="display:flex; justify-content:space-between;"><span>Input P1dB (dBm):</span> <span style="color:#FFABAB">${ip1dbVal}</span></div>
                            </div>`;
                        };

                        const txRaw = editingComp.getRawSpecsForFreq(editingCurrentFreq, "TX");
                        const rxRaw = editingComp.getRawSpecsForFreq(editingCurrentFreq, "RX");

                        txCol.innerHTML += generateChildList("TX") + getSummaryHTML(txRaw, "TX");
                        rxCol.innerHTML += generateChildList("RX") + getSummaryHTML(rxRaw, "RX");

                    } else if (editingComp.isSystem) {
                        txCol.innerHTML += `<div class="spec-grid" style="grid-template-columns: 1fr;"><label style="text-align:left; color:#AAA; font-size:0.8em; margin-bottom:-5px;">Gain (dB)</label><input type="text" id="spec-tx-gain_db"></div>`;
                        rxCol.innerHTML += `<div class="spec-grid" style="grid-template-columns: 1fr;"><label style="text-align:left; color:#AAA; font-size:0.8em; margin-bottom:-5px;">Gain (dB)</label><input type="text" id="spec-rx-gain_db"></div>`;
                    } else {
                        txCol.innerHTML += `
                            <div style="display:flex; flex-direction:column; gap:8px;">
                                <div><label style="display:block; color:#AAA; font-size:0.8em; margin-bottom:2px;">Gain (dB)</label><input type="text" id="spec-tx-gain_db" style="width:100%"></div>
                                <div><label style="display:block; color:#AAA; font-size:0.8em; margin-bottom:2px;">NF (dB)</label><input type="text" id="spec-tx-nf_db" style="width:100%"></div>
                                <div><label style="display:block; color:#AAA; font-size:0.8em; margin-bottom:2px;">P1dB (dBm)</label><input type="text" id="spec-tx-op1db_dbm" style="width:100%"></div>
                            </div>`;
                        
                        rxCol.innerHTML += `
                            <div style="display:flex; flex-direction:column; gap:8px;">
                                <div><label style="display:block; color:#AAA; font-size:0.8em; margin-bottom:2px;">Gain (dB)</label><input type="text" id="spec-rx-gain_db" style="width:100%"></div>
                                <div><label style="display:block; color:#AAA; font-size:0.8em; margin-bottom:2px;">NF (dB)</label><input type="text" id="spec-rx-nf_db" style="width:100%"></div>
                                <div style="opacity:0.3; pointer-events:none;">
                                    <label style="display:block; color:#AAA; font-size:0.8em; margin-bottom:2px;">(P1dB ignored in RX)</label>
                                    <input type="text" disabled value="---" style="width:100%">
                                </div>
                            </div>`;
                    }
                    
                    splitContainer.appendChild(txCol);
                    splitContainer.appendChild(rxCol);
                    dom.modalSpecEditors.appendChild(splitContainer);
            }
        }
        ...
        async function saveComponent() { 
            ...
            // 方法 A: 使用現代 File System Access API (Chrome, Edge, Opera 支援)
            // 這會直接開啟「另存新檔」視窗，允許選擇資料夾
            if ('showSaveFilePicker' in window) {
                try {
                    const handle = await window.showSaveFilePicker({
                        suggestedName: defaultName,
                        types: [{
                            description: 'RF Component File',
                    ...
                    // 成功後不特別跳窗，體驗較流暢，或者可以加一行 console.log
                } catch (err) {
                    // 如果使用者按了「取消」，會觸發 AbortError，我們忽略它
            ...
            // 方法 B: 傳統下載方式 (Firefox, Safari 或舊瀏覽器)
            // 瀏覽器通常會存到預設的「下載」資料夾，或依據瀏覽器設定詢問
            else {
        ...
        // --- Mouse & Canvas Interaction (Updated v10.19) ---
        function onMouseDown(e) {
            ...
            if (e.button === 0) {
                ...
                if (!clickedBlock && !e.ctrlKey && !e.metaKey) {
                    ...
                    selectionData.startX = x; selectionData.startY = y;
                    selectionData.currentX = x; selectionData.currentY = y;
                    return; 
                }
        ...
        function onMouseMove(e) {
            ...
            if (dragData.item) { 
                const dx = x - dragData.offsetX - dragData.item.x;
                const dy = y - dragData.offsetY - dragData.item.y;
                
                blocks.filter(b => b.isSelected).forEach(b => {
                    b.x += dx; b.y += dy;
                });
                
                dragData.offsetX = x - dragData.item.x;
                dragData.offsetY = y - dragData.item.y;

                if (currentCalcMode === "TX" && poutLabels.length) drawPoutLabels(); else drawCanvas(); 
            }
        ...
        function onMouseUp(e) {
            ...
            if (selectionData.isSelecting) {
                ...
                const minX = Math.min(selectionData.startX, selectionData.currentX);
                const maxX = Math.max(selectionData.startX, selectionData.currentX);
                const minY = Math.min(selectionData.startY, selectionData.currentY);
                const maxY = Math.max(selectionData.startY, selectionData.currentY);
                
                blocks.forEach(b => {
                    if (b.x + b.width > minX && b.x < maxX && b.y + b.height > minY && b.y < maxY) {
                        b.isSelected = true;
                        if(isMergeSelectMode && !mergeSelection.includes(b.id)) mergeSelection.push(b.id);
                    }
                });
                drawCanvas();
                return;
        ...
        function onMouseLeave() { dragData.item = null; panData.isPanning = false; lineData.startComp = null; selectionData.isSelecting = false; drawCanvas(); }
        ...
        function drawTextWithBg(ctx, text, x, y, align, color, bgColor) {
            ctx.save(); ctx.font = "bold 12px Consolas"; ctx.textAlign = align; ctx.textBaseline = "middle";
            const metrics = ctx.measureText(text); const padding = 2;
            let rectX = x; if (align === "right") rectX = x - metrics.width; else if (align === "center") rectX = x - metrics.width / 2;
            ctx.fillStyle = bgColor; ctx.fillRect(rectX - padding, y - 6 - padding, metrics.width + padding * 2, 12 + padding * 2);
            ctx.fillStyle = color; ctx.fillText(text, x, y); ctx.restore();
        }

        // --- FIX: Correct Rounded Rectangle Path ---
        ...
        function drawCanvas() {
            ...
            if (poutLabels && poutLabels.length > 0) {
                 poutLabels.forEach(lbl => { drawTextWithBg(ctx, lbl.text, lbl.x, lbl.y, "center", "#FFD700", "rgba(40, 40, 40, 0.8)"); });
            }
            ...
            // Draw Selection Box
            if (selectionData.isSelecting) {
                const w = selectionData.currentX - selectionData.startX;
                const h = selectionData.currentY - selectionData.startY;
                ctx.fillStyle = "rgba(0, 122, 204, 0.2)";
                ctx.strokeStyle = "rgba(0, 122, 204, 0.8)";
                ctx.lineWidth = 1;
                ctx.fillRect(selectionData.startX, selectionData.startY, w, h);
                ctx.strokeRect(selectionData.startX, selectionData.startY, w, h);
            }
            ...
            blocks.forEach(c => {
                ...
                let displayChildren = [];
                if(c.isMerged && c.displayLists) { displayChildren = c.displayLists[lastCalcMode] || c.childrenData; } 
                else if(c.isMerged) { displayChildren = c.childrenData; }
                
                if(c.isMerged) childHeight = 15 + (displayChildren.length * 15);
                c.height = 70 + (specs.length * 15) + childHeight; 
                
                let maxTextW = ctx.measureText(c.name).width;
                ctx.font = "12px Arial"; 
                specs.forEach(l => { maxTextW = Math.max(maxTextW, ctx.measureText(l).width); });
                if(c.isMerged) c.childrenData.forEach(ch => { maxTextW = Math.max(maxTextW, ctx.measureText(ch.name).width); });
                c.width = Math.max(110, maxTextW + 30); 

                ctx.fillStyle = "#00000055"; drawRoundedRect(ctx, c.x+3, c.y+3, c.width, c.height, 8); ctx.fill();
                ctx.fillStyle = mainColor; drawRoundedRect(ctx, c.x, c.y, c.width, c.height, 8); ctx.fill();
                
                // Border Logic (Includes Saturation Red Border)
                if (c.isSelected) { 
                    ctx.strokeStyle = "#00FFFF"; ctx.lineWidth = 3; ctx.setLineDash([5,3]); 
                    drawRoundedRect(ctx, c.x-2, c.y-2, c.width+4, c.height+4, 10); ctx.stroke(); ctx.setLineDash([]); ctx.lineWidth=1; 
                } else if (c.isHighlighted) { 
                    ctx.strokeStyle = "red"; ctx.lineWidth=3; 
                    drawRoundedRect(ctx, c.x-1, c.y-1, c.width+2, c.height+2, 9); ctx.stroke(); ctx.lineWidth=1; 
                } else if (c.isCompressed) {
                    // Saturation Border
                    ctx.strokeStyle = "#FF4444"; ctx.lineWidth = 3;
                    drawRoundedRect(ctx, c.x, c.y, c.width, c.height, 8); ctx.stroke(); ctx.lineWidth = 1;
                } else {
                    ctx.strokeStyle = "#FFFFFF33"; 
                    drawRoundedRect(ctx, c.x, c.y, c.width, c.height, 8); ctx.stroke();
                }

                ctx.save(); ctx.translate(c.x + c.width/2, c.y + 20); drawComponentSymbol(ctx, c); ctx.restore();
                
                ctx.fillStyle = "#111"; ctx.font = "bold 13px Arial"; ctx.textAlign = "center";
                ctx.fillText(c.name, c.x + c.width/2, c.y + 40);
                
                ctx.font = "12px Arial";
                let y = c.y + 58; 
                if (!(c.runtimeResults && c.runtimeResults.freq === lastCalcFreq && c.runtimeResults.mode === lastCalcMode)) {
                     ctx.fillStyle = "#555"; ctx.fillText(c.getDisplaySpecs(), c.x + c.width/2, y - 5);
                }

                specs.forEach(l => {
                    if (l.startsWith("Pin") || l.startsWith("Pout")) ctx.fillStyle = "#FFD700"; else ctx.fillStyle = "#005A9E";
                ...
                if(c.isMerged && c.childrenData.length > 0) {
                    ...
                    if (displayChildren.length === 0 && c.displayLists && lastCalcMode === "RX") {
                         ctx.fillStyle = "#800"; ctx.fillText("(No Chain)", c.x + c.width/2, y); y+=15;
                    } else {
                        ctx.fillStyle = "#111"; ctx.font = "italic 11px Arial";
                        displayChildren.forEach(chData => { 
                            const tempComp = RFComponent.fromDict(chData);
                            const raw = tempComp.getRawSpecsForFreq(lastCalcFreq, lastCalcMode);
                            let valStr = "";
                            if (lastCalcFreq) {
                                if (tempComp.isPassive) valStr = `(L:${formatNum(raw.loss_db || 0, 1)})`;
                                else valStr = `(G:${formatNum(raw.gain_db || 0, 1)})`;
                            }
                            ctx.fillText(`${tempComp.name} ${valStr}`, c.x + c.width/2, y); 
                            y+=15; 
                        });
                    }
                }
                ...
                if (c.runtimeResults && c.runtimeResults.freq === lastCalcFreq && c.runtimeResults.mode === lastCalcMode) {
                    // Saturation Badge
                    if (c.isCompressed) {
                        const badgeText = "⚠️ SAT";
                        ctx.font = "bold 12px Arial";
                        const bw = ctx.measureText(badgeText).width + 10;
                        ctx.fillStyle = "#FF0000"; // Red background
                        drawRoundedRect(ctx, c.x + c.width - bw + 5, c.y - 10, bw, 20, 5);
                        ctx.fill();
                        ctx.fillStyle = "#FFFFFF"; // White text
                        ctx.textAlign = "center";
                        ctx.fillText(badgeText, c.x + c.width - bw/2 + 5, c.y + 4);
                    }

                    const pinT = `Pin: ${formatNum(c.runtimeResults.pin_dbm, 1)}`;
                    const poutT = `Pout: ${formatNum(c.runtimeResults.pout_dbm, 1)}`;
                    drawTextWithBg(ctx, pinT, c.x - 15, c.y + c.height/2, "right", "#FFD700", "rgba(0,0,0,0.6)");
                    drawTextWithBg(ctx, poutT, c.x + c.width + 15, c.y + c.height/2, "left", "#FFD700", "rgba(0,0,0,0.6)");
                }
            });
            ctx.restore();
        }

        function drawComponentSymbol(ctx, comp) {
            ctx.strokeStyle = "#333"; ctx.lineWidth = 1.5; ctx.fillStyle = "transparent";
            if (comp.name.includes("LNA")) { ctx.beginPath(); ctx.moveTo(-8, -8); ctx.lineTo(8, 0); ctx.lineTo(-8, 8); ctx.closePath(); ctx.stroke(); ctx.beginPath(); ctx.moveTo(-4, -5); ctx.lineTo(-4, 5); ctx.stroke(); } 
            else if (comp.name.includes("PA")) { ctx.beginPath(); ctx.moveTo(-8, -8); ctx.lineTo(8, 0); ctx.lineTo(-8, 8); ctx.closePath(); ctx.stroke(); }
            else if (comp.name.includes("Mixer")) { ctx.beginPath(); ctx.arc(0, 0, 8, 0, Math.PI * 2); ctx.stroke(); ctx.beginPath(); ctx.moveTo(-5.6, -5.6); ctx.lineTo(5.6, 5.6); ctx.moveTo(5.6, -5.6); ctx.lineTo(-5.6, 5.6); ctx.stroke(); }
            else if (comp.name.includes("IC")) { ctx.strokeRect(-8, -8, 16, 16); ctx.beginPath(); ctx.moveTo(-8, -4); ctx.lineTo(-12, -4); ctx.moveTo(-8, 4); ctx.lineTo(-12, 4); ctx.moveTo(8, -4); ctx.lineTo(12, -4); ctx.moveTo(8, 4); ctx.lineTo(12, 4); ctx.stroke(); }
            else if (comp.name.includes("Filter")) { ctx.beginPath(); ctx.moveTo(-8, 0); ctx.quadraticCurveTo(-4, -6, 0, 0); ctx.quadraticCurveTo(4, 6, 8, 0); ctx.stroke(); }
            else if (comp.name.includes("Atten")) { ctx.strokeRect(-8, -4, 16, 8); ctx.beginPath(); ctx.moveTo(-8, 8); ctx.lineTo(8, -8); ctx.stroke(); }
            else if (comp.name.includes("Antenna")) { ctx.beginPath(); ctx.moveTo(0, 8); ctx.lineTo(0, 0); ctx.lineTo(-8, -8); ctx.moveTo(0, 0); ctx.lineTo(8, -8); ctx.stroke(); }
            else if (comp.name.includes("Array")) { ctx.strokeRect(-8, -8, 16, 16); ctx.beginPath(); ctx.moveTo(-8, 0); ctx.lineTo(8, 0); ctx.moveTo(0, -8); ctx.lineTo(0, 8); ctx.stroke(); }
            else if (comp.name.includes("Div")) { ctx.strokeRect(-6, -8, 12, 16); ctx.beginPath(); ctx.moveTo(-6, 0); ctx.lineTo(6, -4); ctx.moveTo(-6, 0); ctx.lineTo(6, 4); ctx.stroke(); }
            else { ctx.strokeRect(-6, -6, 12, 12); }
        }

        function drawArrow(x1,y1,x2,y2) {
            const a = Math.atan2(y2-y1, x2-x1); ctx.beginPath(); ctx.moveTo(x1,y1); ctx.lineTo(x2,y2); ctx.stroke();
            ctx.beginPath(); const arrowSize = 15; 
            ctx.moveTo(x2, y2); ctx.lineTo(x2 - arrowSize * Math.cos(a - Math.PI/6), y2 - arrowSize * Math.sin(a - Math.PI/6));
            ctx.lineTo(x2 - arrowSize * Math.cos(a + Math.PI/6), y2 - arrowSize * Math.sin(a + Math.PI/6));
        ...
        function init() {
            ...
            if(dom.t0Label) dom.t0Label.textContent = `T0 (K): ${calculator.T0}`;
            ...
            try {
                const existingExport = document.getElementById('export-button');
                if(!existingExport) {
                    ...
                    dom.exportButton = existingExport;
                }
            } catch (e) {}

            window.addEventListener('resize', resizeCanvas); 
            window.addEventListener('keydown', onKeyDown); // Keyboard Shortcuts
            ...
            document.getElementById('add-ic').addEventListener('click', () => addBlock("IC", false, false, {'1.0': {'TX': {'gain_db': 10, 'nf_db': 3, 'op1db_dbm': 25}, 'RX': {'gain_db': 10, 'nf_db': 3, 'op1db_dbm': 25}}})); 
